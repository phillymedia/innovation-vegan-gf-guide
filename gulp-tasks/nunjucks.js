...
gallerydata.Sheet1.forEach(photo => {
  ...
  if (photo["url"] != undefined && photo["name"] != undefined) {
    //catch error if no actual photo link in this row (no photo data)
    ...
    if (allrestaurantdata[name]) {
      ...
      console.log(
        "Photo with name",
        photo["name"],
        "does not match anything in copy"
      );
    }
  }
...
rdata.Sheet1.forEach(resto => {
  //resto is dict of data about the restaurant

  if (resto.Name) {
    //catch error if no data in this row
    ...
    */
    ...
    if (resto.Neighborhood) {
      ...
      if (!(namecode in n_order_obj)) {
        //set default order and put into
